...
void insercaoPorCor(Pokemon poke[], int cor, int h, int tamanho){
    for (int i = (h + cor); i < tamanho; i+=h)
    {
        Pokemon tmp = poke[i];
        int j = i - h;
        // Comparar pelo peso primeiro, e pelo nome em caso de empate
        while ((j >= 0) &&
               (poke[j].weight > tmp.weight ||
               (poke[j].weight == tmp.weight && strcmp(poke[j].name, tmp.name) > 0))) {
            poke[j + h] = poke[j];
            j -= h;
        }
        poke[j + h] = tmp;
...
void ordenaShellsort(Pokemon poke[], int tamanho){
    int h = 1;
    do
    {
        h = (h * 3) + 1;
    ...
    do
    {
        ...
        {
            insercaoPorCor(poke, cor, h, tamanho);
        }
    }
    while (h != 1);
}
