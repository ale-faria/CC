...
void ordenaInsercaoParcial(Pokemon poke[], int k, int tamanho){
    for (int i = 1; i < tamanho; i++) {
        Pokemon tmp = poke[i];
        int j = (i < k) ? i - 1 : k - 1;
        while (j >= 0 && (
            (strcmp(poke[j].captureDate, tmp.captureDate) > 0) ||
            (strcmp(poke[j].captureDate, tmp.captureDate) == 0 && strcmp(poke[j].name, tmp.name) > 0)
        )) {
            poke[j + 1] = poke[j];
            j--;
        }
...
int main(void) {
    ...
    ordenaInsercaoParcial(poke, 10, position);
