...
void ordena(Pokemon poke[], int tamanho){
    for (int i = 0; i < tamanho - 1; i++) {
        int menor = i;
            for (int j = i + 1; j < tamanho; j++) {
                if (strcmp(poke[j].name, poke[menor].name) < 0) {
                    menor = j;
                }
            }
            swap(poke, menor, i);
...
int pesquisaBinaria(Pokemon poke[], char * entrada, int tamanho){
    int dir = tamanho - 1, esq = 0, meio;

    while (esq <= dir) {
        meio = (esq + dir) / 2;
        int comp = strcmp(poke[meio].name, entrada);
        if (comp == 0) {
            return 1;
        } else if (comp < 0) {
            esq = meio + 1;
...
int main(void) {
    ...
    // Ordena o array por nome, para aplicar a pesquisa binaria em seguida
    ordena(poke, position);
    ...
    while(1){
        ...
        int resp = pesquisaBinaria(poke, entrada, position);
        if(resp == 1){
            printf("SIM\n");
        } else {
            printf("NAO\n");
