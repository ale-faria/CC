...
void quicksort(Pokemon poke[], int esq, int dir)
{
    int i = esq, j = dir;
    Pokemon pivo = poke[(esq+dir)/2];
    while (i <= j) {
        // Comparar pelo generation e, em caso de empate, pelo name
    ...
    if (esq < j)
        quicksort(poke, esq, j);
    if (i < dir)
        quicksort(poke, i, dir);
}
